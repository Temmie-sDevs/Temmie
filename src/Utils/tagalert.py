...
def prepare_text_message(merged: list[LFDto], author_id: str) -> str:
    lines = []
    ...
    for dto in merged:
        ...
        users_str = ", ".join(f"<@{user_id}>" for user_id in dto.users)
...
def tagalert(db: Database, message: discord.Message, tag: str) -> str:
    ...
    return prepare_text_message(merged, message.author.id)